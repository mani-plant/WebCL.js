...
function glGetUboIndex(gl, uboName, program) {
...
function FragmentCircuit(gpu, inpParamsGroup, opParamsGroup, code,  { libCode = '', pixelCode = '', fullFragmentCode = null, shaderParamsType = null }){
    const inpParams = inpParamsGroup.params;
    const opParams = opParamsGroup.params;
    const gl = gpu.gl;
...
function Circuit(gpu, inpParamsGroup, opParamsGroup, code,  { libCode = '', pixelCode = '', fullFragmentCode = null, shaderParamsType = null, uboInIndex = 0, uboOutIndex = 1, setParamsData = true }) {
    const gl = gpu.gl;
    const viewPortSources = gpu.typesInfo.viewPortSources;
    ...
    const inpParams = inpParamsGroup.params;
    const opParams = opParamsGroup.params;
    const vertexCircuit = new VertexCircuit(gpu);
    const fragmentCircuit = new FragmentCircuit(gpu, inpParamsGroup, opParamsGroup, code, { libCode, pixelCode, fullFragmentCode, shaderParamsType });
    const program = glCreateProgram(gl, vertexCircuit.shader, fragmentCircuit.shader);
    ...
    if (shaderParamsType == shaderParamsTypes.ubo) {
        let inIndex = null;
        let outIndex = null;
        this.setUboBindPoints = function ({ newUboInIndex = null, newUboOutIndex = null }) {
        ...
        this.getUboBindPoint = function () {
            return { uboInIndex, uboOutIndex };
        }
        if (inpParams.length > 0) {
            inIndex = glGetUboIndex(gl, "LatticeParamsIn", program);
            // const uboIn = inpParamsGroup.getUbo();
            // uboBindPointsManager.setBindPoint(uboIn, uboInIndex);
            setParamsData && inpParamsGroup.getUbo(uboInIndex);
        }
        outIndex = glGetUboIndex(gl, "LatticeParamsOut", program);
        setParamsData && opParamsGroup.getUbo(uboOutIndex);
        // const uboOut = opParamsGroup.getUbo();
        // uboBindPointsManager.setBindPoint(uboOut, uboOutIndex);
        this.setUboBindPoints({
            newUboInIndex: uboInIndex,
            newUboOutIndex: uboOutIndex
        });
    } else if (shaderParamsType == shaderParamsTypes.uniform) {
        glSetCurrentProgram(gl, program);
        let baseParamsInLoc, shapeParamsInLoc, baseParamsOutLoc, shapeParamsOutLoc;
        if (inpParams.length > 0) {
            baseParamsInLoc = gl.getUniformLocation(program, "_webcl_baseParamsIn");
            shapeParamsInLoc = inpParams.map((x, i) => gl.getUniformLocation(program, "_webcl_shapeParamsIn" + i));
            ...
            // }
            setParamsData && inpParamsGroup.getUniformData({
                baseParamsLoc: baseParamsInLoc,
                shapeParamsLoc: shapeParamsInLoc
            });
        }
        baseParamsOutLoc = gl.getUniformLocation(program, "_webcl_baseParamsOut");
        shapeParamsOutLoc = opParams.map((x, i) => gl.getUniformLocation(program, "_webcl_shapeParamsOut" + i));
        ...
        // }
        setParamsData && opParamsGroup.getUniformData({
            baseParamsLoc: baseParamsOutLoc,
            shapeParamsLoc: shapeParamsOutLoc
        });
        this.getUniformLoc = function () {
            return { baseParamsInLoc, shapeParamsInLoc, baseParamsOutLoc, shapeParamsOutLoc };
        }
        this.setUniformData = function ({ inpParamsGroup, opParamsGroup }) {
            inpParamsGroup !== undefined && baseParamsInLoc !== undefined && inpParamsGroup.getUniformData({
                baseParamsLoc: baseParamsInLoc,
                shapeParamsLoc: shapeParamsInLoc
            });
            opParamsGroup !== undefined && baseParamsOutLoc !== undefined && opParamsGroup.getUniformData({
                baseParamsLoc: baseParamsOutLoc,
                shapeParamsLoc: shapeParamsOutLoc
            });
    ...
    gl.bindBuffer(gl.ARRAY_BUFFER, vertexCircuit.buffers.textureBuffer);
    gl.enableVertexAttribArray(aTexture);
    gl.vertexAttribPointer(aTexture, 2, gl.FLOAT, false, 0, 0);
    gl.bindBuffer(gl.ARRAY_BUFFER, vertexCircuit.buffers.positionBuffer);
    gl.enableVertexAttribArray(aPosition);
    gl.vertexAttribPointer(aPosition, 2, gl.FLOAT, false, 0, 0);
    gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, vertexCircuit.buffers.indexBuffer);

    this.setInpIndices = function (indices) {
        indices = indices || v_texture.keys();
        glSetCurrentProgram(gl, program);
        for (let i = 0; i < indices.length; i++) {
            gl.uniform1i(v_texture[i], indices[i]);
        }
    }
    this.setInpIndices();
    ...
    this.exec = function (inp, op, {
        ...
        if (transferOutput) {
            this.transferOutput(op, transferIndices);
        }
        if (previewIndex !== null && previewIndex < op.length) {
            this.preview(op, previewIndex, { forcePreviewViaProgram, forceNewProgram, previewViewport, previewViewportSource, canvasViewport, canvasViewportSource });
        }
    }
    this.setViewport = function ({ op, viewPortSource = viewPortSources.default, viewport = null }) {
    ...
    this.setFbo = function (fbo) {
        gl.bindFramebuffer(gl.FRAMEBUFFER, fbo);
    }
    this.setVao = function () {
        gl.bindVertexArray(vao);
    }
    this.use = function () {
        gl.useProgram(program);
    }
    this.fastExec = function () {
        gl.drawElements(gl.TRIANGLES, 6, gl.UNSIGNED_SHORT, 0);
    }
    this.previewFbo = function(fbo, {
        previewIndex=0,
        previewViewport = getViewport(gpu, viewPortSources.params, null, opParams, null),
        canvasViewport = getViewport(gpu, viewPortSources.canvas, null, opParams, null)
    }){
    ...
    this.transferOutput = function (op, transferIndices = null) {
    ...
    this.preview = function (op, previewIndex, {
        forcePreviewViaProgram = false, forceNewProgram = false,
        previewViewport = null, previewViewportSource = viewPortSources.default,
        canvasViewport = null, canvasViewportSource = viewPortSources.canvas
    }) {
        previewViewport = getViewport(gpu, previewViewportSource, op, opParams, previewViewport);
        canvasViewport = getViewport(gpu, canvasViewportSource, op, opParams, canvasViewport);
        ...
        if (shaderDataFormat == gl.FLOAT && !forcePreviewViaProgram) {
            ...
            this.previewViaProgram(op, previewIndex, { forceNewProgram, previewViewport, previewViewportSource, canvasViewport, canvasViewportSource });
        }
    }
    this.previewViaProgram = function (op, previewIndex, {
        ...
        if (!drawProg || forceNewProgram) {
            drawProg = new Circuit([forceNewProgram ? op[previewIndex].params : opParams[previewIndex]], [],
                ``,
                ``,
                `
