...
function prepareFragmentShaderCode(gpu, inpParams, opParams, code, { libCode = '', pixelCode = '', fullFragmentCode = null }){
    ...
    ${inpParams.length > 0 ?
        `layout(std140) uniform LatticeParamsIn {
            float _webcl_inpSize[${inpLength}];
            float _webcl_inpStride[${inpLength}];
            float _webcl_sizeI[${inpLength}];
            ${inpShapeStride.map((shapeStride,i) => `float _webcl_shapeStrideIn${i}[${shapeStride.length}];\n`)}
            ${inpShapes.map((shape,i) => `float _webcl_shapeIn${i}[${shape.length}];\n`)}
        };` : ''
    }
    layout(std140) uniform LatticeParamsOut {
        float _webcl_opSize[${opLength}];
        float _webcl_opStride[${opLength}];
        float _webcl_sizeOp[${opLength}];
        ${opShapeStride.map((shapeStride,i) => `float _webcl_shapeStrideOut${i}[${shapeStride.length}];\n`)}
        ${opShapes.map((shape,i) => `float _webcl_shapeOut${i}[${shape.length}];\n`)}
    };
    ...
    ${//inpShapeStride.map((shapeStride,i) => `float _webcl_shapeStrideIn${i}[${shapeStride.length}] = float[](${shapeStride.join('.,')}.);`).join('\n')}
    //${opShapeStride.map((shapeStride,i) => `float _webcl_shapeStrideOut${i}[${shapeStride.length}] = float[](${shapeStride.join('.,')}.);`).join('\n')}
    //${inpShapes.map((shape,i) => `float _webcl_shapeIn${i}[${shape.length}] = float[](${shape.join('.,')}.);`).join('\n')}
    //${opShapes.map((shape,i) => `float _webcl_shapeOut${i}[${shape.length}] = float[](${shape.join('.,')}.);`).join('\n')}
    ''}
    ...
    #define _webcl_getTexIndex() ( (floor(gl_FragCoord.y))*_webcl_sizeO + floor(gl_FragCoord.x) )
    ...
    ''}
    ${inpSize.map((x, i) => `#define _webcl_readInFlat${i}(i) texelFetch(_webcl_uTexture${i}, ( ivec2 ( mod ( floor ( i/_webcl_inpStride[${i}] ), _webcl_sizeI[${i}] ), floor ( floor ( i/_webcl_inpStride[${i}] )/_webcl_sizeI[${i}] ) ) ), 0 ) [int(mod(i, _webcl_inpStride[${i}]))]`).join('\n')}
...
function Circuit(gpu, inpParams, opParams, code, { libCode = '', pixelCode = '', fullFragmentCode = null }) {
    const gl = gpu.gl;
    const viewPortSources = gpu.typesInfo.viewPortSources;
    ...
    //     ];
    // }
    const fragmentShaderCode = prepareFragmentShaderCode(gpu, inpParams, opParams, code, { libCode, pixelCode, fullFragmentCode });
    ...
    if(inpParams.length > 0){
        const inIndex = gl.getUniformBlockIndex(program, "LatticeParamsIn");
        if (inIndex === gl.INVALID_INDEX) {
            console.error("Could not find LatticeParamsIn block.");
        }
        gl.uniformBlockBinding(program, inIndex, 0);
        ...
        // );
        const blockInParams = clUtils.getBlockData(inpParams);
        // UBO for LatticeParamsIn
        gl.bindBuffer(gl.UNIFORM_BUFFER, uboIn);
        gl.bufferData(gl.UNIFORM_BUFFER, blockInParams, gl.STATIC_DRAW);
        // gl.bufferSubData(gl.UNIFORM_BUFFER, 0, data);
        gl.bindBufferBase(gl.UNIFORM_BUFFER, 0, uboIn);
    }
    const outIndex = gl.getUniformBlockIndex(program, "LatticeParamsOut");
    if (outIndex === gl.INVALID_INDEX) {
        console.error("Could not find LatticeParamsOut block.");
    }
    gl.uniformBlockBinding(program, outIndex, 1);
