...
function prepareFragmentShaderCode(gpu, inpParams, opParams, code, { libCode = '', pixelCode = '', fullFragmentCode = null }){
    ...
    ${inpParams.length > 0 ?
        `layout(std140) uniform LatticeParamsIn {
            ...
            vec4 _webcl_baseParamsIn[${opLength}]; // x = size, y = stride, z = texSize
    ...
    ${inpSize.map((x, i) => `#define _webcl_readInFlat${i}(i) texelFetch(_webcl_uTexture${i}, ( ivec2 ( mod ( floor ( i/_webcl_baseParamsIn[${i}].y ), _webcl_baseParamsIn[${i}].z ), floor ( floor ( i/_webcl_baseParamsIn[${i}].y )/_webcl_baseParamsIn[${i}].z ) ) ), 0 ) [int(mod(i, _webcl_baseParamsIn[${i}].y))]`).join('\n')}
