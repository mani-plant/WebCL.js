...
function generateIndexMacro(params, suffix = '') {
	...
	macro += shape.map((_, i) =>
		`(i${i}) * _webcl_shapeStride${suffix}[${i}]`
	).join(' + ');
...
function generateShapedIndexMacro(params, suffix = '') {
	...
	for (let i = 0; i < shape.length; i++) {
		if (i < shape.length - 1) {
			macro += `    shaped_index[${i}] = floor(_rem / (_webcl_shapeStride${suffix}[${i}])); \\\n`;
			macro += `    _rem = mod(_rem, (_webcl_shapeStride${suffix}[${i}])); \\\n`;
		} else {
...
function generateNextShapedIndexMacro(params, suffix = '') {
	...
	for (let d = dims - 1; d >= 0; d--) {
		...
		macro += `        shaped_index[${d}] = mod(_tmp, (_webcl_shape${suffix}[${d}])); \\\n`;
		// Carry is the quotient of _tmp divided by shape[d]
		macro += `        _carry = floor(_tmp / (_webcl_shape${suffix}[${d}])); \\\n`;
		macro += "    } \\\n";
	}
...
// }
function getBlockData(params){
	let data = [];
	for(let p of params){
		data.push(p.size, 0, 0, 0);
	}
	for(let p of params){
